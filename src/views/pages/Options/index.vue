...
onMounted(async () => {
    ...
    productService.getoptions().then((data) => { options.value = data });
});
...
const saveProduct = () => {
    ...
    productService.addOption(title.value).then((response) => { 
        console.log(response)
        if(response.status){
            toast.add({ severity: 'success', summary: 'Option Added Successfully.', detail: 'Message Content', life: 3000 });
        }else
        {
    ...
    <div class="grid">
        <div class="col-12">
            <div class="card">
                ...
                <Dialog v-model:visible="productDialog" :style="{ width: '450px' }" header="Add new Option" :modal="true" class="p-fluid">
                    <div class="field">
                        <label for="name">Title</label>
                        <InputText id="name" v-model="title" required="true" autofocus />
                        <small class="p-invalid" v-if="submitted && !title">Title is required.</small>
                    </div>
