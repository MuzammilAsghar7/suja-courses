...
const pageTitle = ref('Add Inner Lesson');
...
const saveLesson = () => {
    // if(InnerlessonId){
        // lessonService.updateInnerLesson(lessonId,lesson.value)
        // .then((data) => { 
        //     loading.value=false;  
        ...
        // });
    // }else{
        lessonService.addInnerLesson(lesson.value, lessonId).then((response) => { 
            console.log(response);
            toast.add({ severity: 'success', summary: 'Added', detail: 'Inner Lesson has been added Successfully.', life: 3000 })
            router.push(`/chapters/${chapterId}/lessons`);
